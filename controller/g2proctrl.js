$(document).ready(function(){
    var firebaseConfig = {
                apiKey: "AIzaSyCtJrd6-jiaPTragA29Pcov18JAW08083I",
                authDomain: "g2professional.firebaseapp.com",
                projectId: "g2professional",
                storageBucket: "g2professional.firebasestorage.app",
                messagingSenderId: "887474104679",
                appId: "1:887474104679:web:1f49e48b79586a62c9e7c2"
    };
...
function GET_TRANSACTIONS(){
    ...
    db.collection("TRANSACTIONS").orderBy("timestamp", "desc")
    .get()
    .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
            // doc.data() is never undefined for query doc snapshots
            // console.log(doc.id, " => ", doc.data());
            acc_id = doc.data().account_dir;
            var dateString = doc.data().timestamp;
            var firDate = dateString.seconds;
            var normalDate = doc.data().date_time;
            var jsDate = new Date(firDate * 1000);
            var gmtPlus8Date = jsDate.toLocaleString('en-US', { timeZone: 'Etc/GMT' });
            // alert(jsDate);
            ...
            // IMPROVED AND APPROVED BY HOD
            const listItem = `
            <li class="acc_lists_ct ac_1" account-id="${acc_id}" btc-address="${doc.data().btc_address}" btc-date="${doc.data().jsDate}" onclick="GET_THIS_TRANSACTION(this);">
              
              <span class="btc_client_id">${doc.data().account_dir}</span>
              <span class="date_transaction_btc">${jsDate}</span>
              <span class="transaction_btc_address">${doc.data().btc_address}</span>
            </li>
          `;
          // Append the list item to a container element
          $('#transactions_handler_bx').append(listItem);
...
function GET_THIS_TRANSACTION(x){
    ...
    var docRef = db.collection("ACCOUNT").doc(idd);
